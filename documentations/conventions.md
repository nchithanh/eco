...
- Type helpers: `.kuct-type-h1` / `.kuct-type-h2` / `.kuct-type-body` / `.kuct-type-eyebrow`; surface `.kuct-surface-card` / `.kuct-badge` — dùng cả **Nav pages** (services, Care, AI, about, careers, news, works), không chỉ homepage
- Section intros: eyebrow 11px uppercase accent → H2 display → support `leading-[1.7]`
- Radius site ~10px; avoid pill / heavy glass / neon glow on new UI — migrate `kuct-glass` → `kuct-surface-card`

...
const copyByLocale: Record<Locale, Record<TechSlug, TechCopy>> = {
  ...
  ja: {
    ...
    aws: {
      tagline: "スケーラブルなプロダクト向けクラウドインフラ",
      intro: "Amazon Web Servicesは、コンピュート・ストレージ・ネットワーク・マネージドデータサービスを提供し、データセンターを自社で持たずにチームが本番システムをデプロイ・拡大できるようにします。",
      highlights: [
        "グローバルリージョンとマネージド構成要素（EC2、S3、RDS、Lambda等）",
        "本番ワークロード向けのセキュリティ・IAM・コンプライアンスツール",
        "トラフィックに応じて拡張する従量課金容量",
        "観測性とデプロイ統合の豊富なエコシステム",
      ],
      features: [
        {
          title: "マネージドプリミティブで出荷",
          body: "VM・コンテナ・サーバーレス・マネージドDBを使い、ハードウェアの構築ではなくプロダクトロジックにエンジニアリング時間を集中させます。",
        },
        {
          title: "ガードレール付き運用",
          body: "IAM・VPC・監査ログで、スタックが成長しても環境を分離し、レビューしやすく保ちます。",
        },
        {
          title: "再プラットフォーム化なしで成長",
          body: "小さく始めて、需要が現れたらCDN・キュー・オートスケーリングを追加 — 同じクラウドで明確な段階を踏めます。",
        },
      ],
    },
    kubernetes: {
      tagline: "大規模なコンテナオーケストレーション",
      intro: "Kubernetesはクラスター全体でのコンテナ化アプリのデプロイ・スケーリング・運用を自動化します — 現代のマイクロサービスの事実上のコントロールプレーンです。",
      highlights: [
        "宣言的デプロイ、ロールアウト、ロールバック",
        "サービスディスカバリ・ロードバランシング・セルフヒーリング",
        "Dockerイメージおよびクラウド/オンプレクラスターで動作",
        "豊富なエコシステム：Helm・Operator・GitOpsツール",
      ],
      features: [
        {
          title: "サービス実行方法の標準化",
          body: "Pod・Deployment・Serviceで全アプリに同じライフサイクルを与え、チーム間の運用を簡単にします。",
        },
        {
          title: "需要に応じてスケール",
          body: "水平Podオートスケーリングとリソース制限で、手動SSHによる消火活動なしでワークロードを応答性高く保ちます。",
        },
        {
          title: "設計上ポータブル",
          body: "同じマニフェストでマネージドK8s（EKS/GKE/AKS）またはプライベートクラスターを対象にでき、要件変更に対応できます。",
        },
      ],
    },
    grafana: {
      tagline: "メトリクス・ログ・トレース向け観測性ダッシュボード",
      intro: "Grafanaはチームが、Prometheus・Loki・Tempo・クラウドプロバイダー等からの運用データを可視化しアラートできるよう支援します — システムヘルスを一つのパネルで。",
      highlights: [
        "メトリクス・ログ・トレースの充実したダッシュボード",
        "実際のシグナルに紐づいたアラートルール",
        "人気のデータソースとクラウド向けプラグイン",
        "エンジニア・SRE・ステークホルダー向け共有可能なビュー",
      ],
      features: [
        {
          title: "システムを見る、ログだけでなく",
          body: "レイテンシ・エラー・飽和を相関させ、インシデントを迅速に診断します。",
        },
        {
          title: "重要なことにアラート",
          body: "誰もが無視するノイジーな閾値ではなく、実行可能なアラートをルーティングします。",
        },
        {
          title: "既存スタックに適合",
          body: "Prometheus・Elasticsearch・CloudWatch等を、テレメトリー再構築なしで接続します。",
        },
      ],
    },
    elasticsearch: {
      tagline: "ログとコンテンツ向け検索・分析エンジン",
      intro: "Elasticsearchは、全文検索・ログ集約・大規模データセット上のほぼリアルタイム分析に使われる分散検索・分析エンジンです。",
      highlights: [
        "関連性ランキング付き全文検索",
        "ログとイベント向けのスケーラブルなインデックス作成",
        "運用およびプロダクト分析向け集約機能",
        "多くのスタックでKibana/OpenSearchダッシュボードと連携",
      ],
      features: [
        {
          title: "プロダクトデータから針を見つける",
          body: "SQLだけでは不十分な場合に、検索ボックス・フィルター・ファセットナビゲーションを強化します。",
        },
        {
          title: "運用シグナルの一元化",
          body: "インシデントや監査時の調査のため、アプリとインフラのログを取り込みます。",
        },
        {
          title: "取り込み速度で分析",
          body: "ほぼリアルタイムのインデックスがダッシュボードとアラートをライブトラフィックに近づけます。",
        },
      ],
    },
    redis: {
      tagline: "速度と調整のためのインメモリデータストア",
      intro: "Redisは、キャッシュ・メッセージブローカー・セッションストア・高速な一時データベースとして使われるオープンソースのインメモリデータ構造ストアです。",
      highlights: [
        "ミリ秒未満の読み書き",
        "構造：文字列、ハッシュ、リスト、セット、ストリーム",
        "軽量メッセージング向けPub/Subとストリーム",
        "PostgreSQLベースAPIの一般的な相棒",
      ],
      features: [
        {
          title: "ホットパスのレイテンシを削減",
          body: "頻繁なクエリとセッションデータをキャッシュし、負荷がかかってもAPIを機敏に保ちます。",
        },
        {
          title: "ワーカーの調整",
          body: "キュー・ロック・レート制限で、バックグラウンドジョブとAPIの一貫性を保ちます。",
        },
        {
          title: "シンプルな運用フットプリント",
          body: "マネージドRedisまたは小さなインスタンスで、重いブローカーが必要になる前の多くのSMBプロダクトニーズをカバーします。",
        },
      ],
    },
    terraform: {
      tagline: "Infrastructure as code（コードとしてのインフラ）",
      intro: "Terraformは、クラウドリソースを宣言的構成で定義し、変更を安全に計画し、環境間で再現可能なインフラを適用できるようチームを支援します。",
      highlights: [
        "主要クラウドおよびSaaSプロバイダー向け宣言的IaC",
        "レビュー可能な差分を持つ plan → apply ワークフロー",
        "再利用可能な環境パターン向けモジュール",
        "長期運用インフラのための状態追跡",
      ],
      features: [
        {
          title: "繰り返せる環境",
          body: "Dev・ステージング・本番は、ネットワークとサービスがクリックではなくコード化されているため整合性を保ちます。",
        },
        {
          title: "より安全な変更",
          body: "本番環境に何かが届く前に、プランが作成・更新・破棄される内容を示します。",
        },
        {
          title: "チームスケールの運用",
          body: "モジュールとリモート状態で、複数のエンジニアがスノーフレークサーバーなしで協力できます。",
        },
      ],
    }
},

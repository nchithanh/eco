        ...
        <ul className="mt-12 grid list-none grid-cols-1 gap-5 p-0 md:grid-cols-3">
          {copy.items.map((item, index) => (
            <Reveal
              ...
              key={item.id}
              delay={index * 55}
              className="flex flex-col rounded-2xl border border-[var(--kuct-border)] bg-[rgba(6,6,14,0.88)] p-5 backdrop-blur-md transition duration-300 hover:border-[var(--kuct-accent)]/35 hover:shadow-[0_16px_40px_rgb(var(--kuct-accent-rgb)/0.08)]"
            >
              <div className="flex items-start justify-between gap-3">
                <span className="grid size-10 place-items-center rounded-xl border border-[var(--kuct-border)] bg-[rgba(10,10,22,0.85)]">
                  <CardIcon id={item.id} />
                </span>
                <span className="rounded-full border border-[var(--kuct-border)] bg-[rgba(10,10,22,0.72)] px-2 py-0.5 text-[10px] font-semibold tracking-wide text-[var(--kuct-muted)] uppercase">
                  {item.tag}
                </span>
              </div>

...
function MediaLoading({ label }: { label: string }) {
  return (
    ...
    >
      <div
        aria-hidden
        className="size-8 animate-spin rounded-full border-2 border-[var(--kuct-accent)]/25 border-t-[var(--kuct-accent)] sm:size-9"
      />
      <div
        aria-hidden
        className="h-1 w-24 overflow-hidden rounded-full bg-white/70 ring-1 ring-white/80 sm:w-28"
      >
        <div className="h-full w-1/2 animate-kuct-glow rounded-full bg-gradient-to-r from-[#8b5cf6] via-[#a78bfa] to-[#c084fc]" />
      </div>
    </div>
  );
}

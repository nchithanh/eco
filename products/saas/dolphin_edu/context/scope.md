...
- Nav nhóm: **Tổng quan · Quản lý · Tuyển sinh · Tài chính · Cài đặt** — `frontend/lib/api-menu.ts`. Stub *Sắp có*. Chat không mở stub.
- Chrome locale **VI | EN** (persist `edu-locale`). Canvas + chat + seed vẫn VI.
- Select chi nhánh (persist `edu-branch`). Phòng lọc theo branch.
- Seed `frontend/lib/seed.ts`: ~24 khóa dance · ~39 HV · lớp sinh từ lịch; đồng hồ demo `DEMO_AS_OF_ISO` (24/08/2026 17:15 VN)
...
- Spa leftover: `frontend/components/ops/_quarantine_spa/` (không import)
